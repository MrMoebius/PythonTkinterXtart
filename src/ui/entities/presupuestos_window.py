...
class PresupuestosWindow(BaseCRUDWindow):
    """Ventana para gestionar presupuestos"""
    ...
    def __init__(self, parent, api: RESTClient):
        ...
        super().__init__(parent, api, "presupuestos", columns, filters)
    ...
    def _show_form(self, item: Optional[Dict]):
        ...
        form_window.title("Nuevo Presupuesto" if not item else "Editar Presupuesto")
        ...
        for i, field_config in enumerate(form_fields):
            ...
            if field_config["type"] == "select":
                if field_config["name"] == "cliente_id":
                    ...
                    combo = ttk.Combobox(main_frame, values=["PENDIENTE", "APROBADO", "RECHAZADO"], 
        ...
        def save():
            ...
            # Guardar
            if item:
                result = self.api.update("presupuestos", item["id"], data)
            else:
                result = self.api.create("presupuestos", data)
